...
public class testessin {
	...
	static char tabRectangle[][] = new char[30][50];// Remplir tableau
...
//----------------------------------------------------------------------
		...
		dessinRectangle(24, 22, 6, 4);

		dessinRectangle(13, 39, 18, 3);
		dessinRectangle(3, 34, 12, 12);
		dessinRectangle(1, 1, 9, 9);
		dessinRectangle(10, 1, 3, 1);

		// dessinRectangle(7,8);

		for (int x = 0; x < tabRectangle.length; x++) {// afficher le tableau
	...
	static public void dessinRectangle(int pX, int pY, int longeur, int largeur) {
		...
		for (int y = 0; y < largeur; y++) {
			tabRectangle[0 + (pX - 1)][y + (pY - 1)] = '*';
		}

		for (int x1 = 0; x1 < longeur; x1++) {

			tabRectangle[x1 + (pX - 1)][largeur - 1 + (pY - 1)] = '*';
		}

		for (int y1 = 0; y1 < largeur; y1++) {
			tabRectangle[longeur - 1 + (pX - 1)][y1 + (pY - 1)] = '*';
		}
	}
}
